    ...
    def executar_reconhecimento(self):
        ...
        try:
            while True:
                ...
                resultados = self.utilitarios.maos.process(
                ...
                if resultados.multi_hand_landmarks:
                    ...
                    side = int(min(altura, largura) * 0.25)
                    cx, cy = largura // 2, altura // 2
                    half = side // 2
                    limite_esquerdo = cx - half
                    limite_superior = cy - half
                    limite_direito = cx + half
                    limite_inferior = cy + half

                    for marcos_mao in resultados.multi_hand_landmarks:
                        self.utilitarios.desenhar_landmarks(quadro, marcos_mao)

                        xs = [marco.x for marco in marcos_mao.landmark]
                        ys = [marco.y for marco in marcos_mao.landmark]
                        centro_x = int((min(xs) + max(xs)) * largura / 2)
                        centro_y = int((min(ys) + max(ys)) * altura / 2)
                        mao_no_indicador = (
                            limite_esquerdo <= centro_x <= limite_direito and
                            limite_superior <= centro_y <= limite_inferior
                        )

                        if not mao_no_indicador:
                            continue

                        caracteristicas = self.utilitarios.extrair_caracteristicas(
                            marcos_mao
                        )
                        previsao_atual, confianca = self.classificador.prever(
                            caracteristicas
                        )
                        self._atualizar_historico(previsao_atual, confianca)
                        indicator_color = (
                            (0, 255, 0) if previsao_atual != "?" else (255, 0, 0)
                        )

                        if confianca > 0.8 and previsao_atual != "?":
                            mesma_classe = getattr(
                                self, '_ultima_classe_enviada', None
                            ) == previsao_atual
                            passou_intervalo = (
                                time.time() - getattr(
                                    self, '_tempo_ultima_classe', 0
                                ) > 3.0
                            )
                            if not mesma_classe or passou_intervalo:
                ...
                self._desenhar_interface(
                    quadro, previsao_atual, confianca, indicator_color
                )
